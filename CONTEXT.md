...
| **Integers** | The Primitive that yields integers from an unbounded sequence; three factory methods expose different domains — `natural()` for ℕ (0, 1, 2, …), `from(int $start)` for any upward sequence starting at `$start`, and `all()` for ℤ interleaved as 0, 1, −1, 2, −2, …. All three are Infinite Sources. |
| **Infinite Source** | A Generator whose sequence has no upper bound and never terminates on its own; identified at runtime by `isInfinite()` returning `true`. Safe to compose with `filter`, `map`, `merge`, and `take`; composing with `product` or `repeat` throws `\LogicException` immediately. The infinite trait propagates through safe combinators and is sealed only by `take`. |
| **Take** | The combinator that truncates any Generator to at most `$n` values; the only combinator that converts an Infinite Source into a finite sequence. Throws `\InvalidArgumentException` at construction if `$n < 1`. If the upstream source has fewer than `$n` values, it yields what exists and stops without error. |
